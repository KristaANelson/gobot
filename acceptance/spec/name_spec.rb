  ...
  describe '/names' do
    context 'with no params' do
      it 'returns a single name with no params' do
        uri = URI.parse("http://localhost:9999/names/random")
        response = Net::HTTP.get_response(uri)

        expect(response.code).to eq('200')
        res = JSON.parse(response.body)
        expect(res.length).to eq(1)
        expect(res['names'][0]).to be
      end

      context 'with a limit param' do
        it 'it returns multiple names' do
          uri = URI.parse("http://localhost:9999/names/bomb/3")
          response = Net::HTTP.get_response(uri)

          expect(response.code).to eq('200')
          expect(JSON.parse(response.body)['names'].length).to eq(3)
        end
      end
